...
# guessing,
# shrinkage on metric variables' coefficients.
#
# ## Data
...
mcmc = MCMC(kernel, num_warmup=1000, num_samples=10000, num_chains=4)
mcmc.run(
    random.PRNGKey(0),
...
idata = az.from_numpyro(
    mcmc,
    coords=dict(metric=X_df.columns,
                group=ej.cat.categories,
                interactions=X_interactions_df.columns),
    dims=dict(_a=['metric'], _aG=['group'], _aInteraction=['interactions']),
)
...
X_test_interactions_df = create_interaction_terms_between(X_test_df, features)
