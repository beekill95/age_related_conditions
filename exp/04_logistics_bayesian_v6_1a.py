...
all_df = pd.concat([X_df, X_interactions_df], axis=1)
all_df = pd.DataFrame(
...
def cross_validation(X_df, y, ej, n_folds: int = 10):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X_df, y)):
        ...
        Xtr_df = filter_correlated_features(Xtr_df, correlation_threshold=0.7)
