...
# to a small subspace.
# Unlike the first version, this version will add a regularization term:
# the embedded feature space should be from normal gaussian.
# _(The reason for this being in the 2nd experiment,
# scaling the embedded feature seems to improve the performance of the model.
# Thus, I'm curious to see if this regularization has any benefit?)_
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x, mode: Literal['encoder', 'decoder', 'full'] = 'full'):
        if mode == 'encoder':
...
def create_training_and_evaluation_step(
    ...
    def train_step(dataloader: DataLoader, device: str, epoch: int, progress: bool = True):
        ...
        for i, (X, y) in bar:
            ...
            pred = model(encoder_output, mode='decoder')

            # Losses.
            encoder_loss = encoder_regularization_loss(encoder_output)
            decoder_loss = loss_fn(pred, y)

            loss = regularization_weight * encoder_loss + decoder_loss
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(X, y)):
    ...
    Xtr_encoded = autoencoder(Xtr, mode='encoder').detach().cpu().numpy()
    Xtr_encoded = np.concatenate(
            [Xtr_encoded, ej.values[train_idx][..., None]], axis=1)
    Xva_encoded = autoencoder(Xva, mode='encoder').detach().cpu().numpy()
