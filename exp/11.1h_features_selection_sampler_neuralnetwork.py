...
class NNClassifier(nn.Module):
    def __init__(self, input_shape: int) -> None:
        ...
        encoder, decoder = self._create_encoder_decoder_layers(
            input_shape,
            hidden_layers_size)
        ...
        self.output = nn.Sequential(
            nn.LayerNorm(hidden_layers_size[-1]),
            nn.Linear(hidden_layers_size[-1], 1),
        )

    def forward(self, x, *,
                mode: Literal['encoder', 'output', 'decoder', 'full'] = 'full',
                logit: bool = False):
        if mode == 'encoder':
    ...
    def _create_encoder_decoder_layers(
            self, input_size: int, hidden_layers_size: list[int]):
        nb_layers = len(hidden_layers_size)

        def build_encoder():
            for i, size in enumerate(hidden_layers_size):
                if i == nb_layers - 1:
                    yield [
                        nn.Linear(hidden_layers_size[i - 1], size),
                    ]
                else:
                    yield [
                        nn.Linear(
                            input_size if i == 0
                            else hidden_layers_size[i - 1], size),
                        nn.ReLU(),
                        nn.Dropout(),
                        nn.LayerNorm(size),
                    ]
        ...
        def build_decoder():
            layers_reversed = hidden_layers_size[::-1]
            for i, size in enumerate(layers_reversed):
                if i == 0:
                    yield [
                        nn.LayerNorm(size),
                        nn.Linear(size, layers_reversed[1]),
                    ]
                else:
                    yield [
                        nn.ReLU(),
                        ...
                        nn.Linear(
                            size,
                            layers_reversed[i + 1] if i < nb_layers - 1
                            else input_size),
                    ]

        return (list(reduce(operator.add, build_encoder(), [])),
                list(reduce(operator.add, build_decoder(), [])))
...
def create_training_and_evaluation_step(
    ...
    def calc_decoder_loss(Xtrue: torch.Tensor, Xdecoded: torch.Tensor):
        """
        Calculate decoder loss, which is just mean-squared error.
        """
        return torch.nn.functional.mse_loss(Xdecoded, Xtrue)
    ...
    def train_step(dataloader: DataLoader, *,
        ...
        for i, (X, y) in bar:
            ...
            X_noisy = X + train_noise * torch.rand_like(X)

            # Make prediction and calculate loss.
            encoder_output = model(X_noisy, mode='encoder')
            decoder_output = model(encoder_output, mode='decoder')
            pred = model(encoder_output, mode='output', logit=True)

            # Losses.
            encoder_loss = encoder_regularization_loss(encoder_output)
            decoder_loss = calc_decoder_loss(X, decoder_output)
            classification_loss = loss_fn(pred, y)

            loss = (regularization_weight * encoder_loss
                    + decoder_weight * decoder_loss
                    + classification_loss)
            regularization_loss += encoder_loss.item()
            decoder_total_loss += decoder_loss.item()
            ...
            if progress:
                bar.set_postfix_str(
                    ...
                    f'- Decoder loss {decoder_total_loss / (i + 1):.4f}')

        return (train_loss / num_batches,
                regularization_loss / num_batches,
                decoder_total_loss / num_batches)
...
def train(model: nn.Module,
    ...
    for epoch in bar:
        train_loss, train_regu_loss, train_decoder_loss = train_step(
        ...
        bar.set_postfix_str(
            ...
            f'- Train Dec: {train_decoder_loss:.4f}'
